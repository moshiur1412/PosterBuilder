...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    function wrapText($image, $font, $text, $maxWidth)
    {
        ...
        foreach ($words as $word) {
            ...
            $testBox = imagettfbbox(17, 0, $font, $testLine);
    ...
    $wrappedTitle = wrapText($default_image, $bold_font_path, $heading, $maxTitleWidth);

    // Calculate x-coordinate for the title
    $titleX = $uploadedImageX; // Left align by default
    if ($heading_position === 'center') {
        $titleX = ($imageWidth - imagettfbbox(30, 0, $bold_font_path, $wrappedTitle[0])[2]) / 2;
    } elseif ($heading_position === 'right') {
        $titleX = $imageWidth - imagettfbbox(30, 0, $bold_font_path, $wrappedTitle[0])[2] - 75;
    }
    ...
    foreach ($wrappedTitle as $line) {
        imagettftext($default_image, $fontsize, $angle, $titleX, $titleY, $textColor, $bold_font_path, $line);
    ...
    if ($description_position === 'center') {
        $descriptionX = ($imageWidth - imagettfbbox(20, 0, $medium_font_path, $wrappedDescription[0])[2]) / 2;
    } elseif ($description_position === 'right') {
        $descriptionX = $imageWidth - imagettfbbox(20, 0, $medium_font_path, $wrappedDescription[0])[2] - 5;
    }
    ...
    $wrappedDescription = wrapDescriptionText($default_image, $medium_font_path, $description, $maxDescriptionWidth);
    ...
    foreach ($wrappedDescription as $line) {
        imagettftext($default_image, $fontsize, $angle, $descriptionX, $descriptionY, $descriptionColor, $medium_font_path, $line);
        $descriptionY += 5;
        $descriptionBox = imagettfbbox($fontsize, $angle, $medium_font_path, $line);
